...
export class Budget {
  ...
  get availableForDebt(): number {
    return Number(this.user?.monthlyIncome || 0) - this.totalActual;
  }
  ...
  // Calculate available debt budget after all deductions
  calculateAvlDebtBudget(): number {
    const monthlyIncome = Number(this.user?.monthlyIncome || 0);
    const totalExpenses = this.totalBudgeted;
    const homeContribution = Number(this.homeContribution || 0);
    
    return monthlyIncome - totalExpenses - homeContribution;
  }
}