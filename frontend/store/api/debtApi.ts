  ...
  endpoints: (builder) => ({
    ...
    getDebts: builder.query<Debt[], void>({
      ...
      transformResponse: (response: any) => {
        ...
        return debtsArray.map(debt => ({
          ...
          paymentInterval: debt.paymentInterval || 'monthly', // Add payment interval
    ...
    createDebt: builder.mutation<Debt, Partial<Debt>>({
      query: (debt) => {
        ...
        const transformedDebt = {
          ...
          ...(debt.status && { status: debt.status.toUpperCase() }),
          // Ensure debt type is in the correct format
    ...
    >({
      ...
      transformResponse: (response: any) => {
        ...
        return {
          monthlyIncome: Number(budgetData.user?.monthlyIncome || 0),
          homeContribution: Number(budgetData.homeContribution || 0),
          avlDebtBudget: Number(budgetData.avlDebtBudget || 0),
          expenses: {
