...
import { logoutUser, updateUserPreferences, updateUserProfile, clearSavedCredentials } from '@/store/slices/authSlice';
...
export default function ProfileTab() {
  console.log('ProfileTab: Component rendering started');
  
  try {
    const dispatch = useDispatch();
    const { user, isAuthenticated } = useSelector((state: RootState) => state.auth);
    const { data: debtsData, isLoading: debtsLoading, error: debtsError } = useGetDebtsQuery();
    const { data: budgetData, isLoading: budgetLoading, error: budgetError } = useGetBudgetQuery();
    
    console.log('ProfileTab: Redux state - user:', user);
    console.log('ProfileTab: Redux state - isAuthenticated:', isAuthenticated);
    ...
    const monthlyIncome = user?.monthlyIncome || budgetData?.monthlyIncome || 0;
    ...
    console.log('ProfileTab: About to render main content');

    // Show loading state while data is being fetched
    if (debtsLoading || budgetLoading) {
      console.log('ProfileTab: Showing loading state');
      return (
    ...
    if (debtsError || budgetError) {
      console.log('ProfileTab: Showing error state');
      return (
        <View style={styles.container}>
          <StatusBar barStyle="dark-content" backgroundColor={COLORS.white} />
    ...
    const handleCurrencyChange = () => {
      const currencyOptions = AVAILABLE_CURRENCIES.slice(0, 10).map(currency => ({
      ...
      currencyOptions.push({ text: 'Cancel', style: 'cancel' as const });
    ...
    const handleClearSavedCredentials = async () => {
      Alert.alert(
        ...
        'This will remove your saved login credentials. You will need to enter them manually next time you log in.',
        [
          {
            text: 'Cancel',
            style: 'cancel',
          },
          {
            ...
            onPress: async () => {
              try {
                await dispatch(clearSavedCredentials() as any);
    ...
    if (loading) {
      console.log('ProfileTab: Showing loading state for actions');
      return (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={COLORS.primary} />
    ...
    if (!isAuthenticated) {
      console.log('ProfileTab: User not authenticated, showing auth prompt');
      return (
        <View style={styles.container}>
          <StatusBar barStyle="dark-content" backgroundColor={COLORS.white} />
    ...
    console.log('ProfileTab: Rendering main profile content');
    return (
      <View style={styles.container}>
        <StatusBar barStyle="dark-content" backgroundColor={COLORS.white} />
        {/* Header */}
              ...
              <View style={styles.settingsCard}>
                ...
                >
                  <View style={styles.menuItemLeft}>
                    ...
                    <Text style={styles.menuItemText}>Clear Saved Credentials</Text>
                  </View>
                  <ChevronRight size={20} color={COLORS.textLight} />
                </TouchableOpacity>
              </View>
    ...
    console.error('ProfileTab error:', error);
    return (
      <View style={styles.container}>
        <StatusBar barStyle="dark-content" backgroundColor={COLORS.white} />
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorSubtext}>Please try again later</Text>
          <Text style={styles.errorSubtext}>Error: {error?.toString()}</Text>
        </View>
      </View>
    );
  }
