...
import { loginUser, saveCredentials, getSavedCredentials, clearSavedCredentials } from '@/store/slices/authSlice';
import { router } from 'expo-router';
import { Eye, EyeOff, Mail, Lock, ArrowLeft, UserPlus, CheckSquare, Square } from 'lucide-react-native';
...
export default function LoginScreen() {
  ...
  const { isLoading, error, savedCredentials, credentialsSaved } = useSelector((state: RootState) => state.auth);
  ...
  useEffect(() => {
    const loadSavedCredentials = async () => {
      const result = await dispatch(getSavedCredentials() as any);
      if (getSavedCredentials.fulfilled.match(result) && result.payload) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (loginUser.fulfilled.match(result)) {
        ...
        // Save credentials if option is selected
        if (shouldSaveCredentials) {
          await dispatch(saveCredentials({ email, password }) as any);
        } else if (credentialsSaved) {
          // Clear saved credentials if user unchecks the option
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.saveCredentialsContainer}>
            ...
            >
              ...
              <Text style={styles.saveCredentialsText}>Save credentials for faster login</Text>
            </TouchableOpacity>
          </View>
